...
import {
  clientsApi,
  sacmedConfirmationsApi,
  Client,
  SacmedConfirmationConfig,
  SacmedPendingConfirmation,
  SacmedConfirmationStatus,
  CreateSacmedConfirmationConfigDto,
} from '@/lib/api';
import {
  FiArrowLeft,
  FiLoader,
...
export default function SacmedConfirmationsPage() {
  const params = useParams();
  const clientId = params?.id as string;

  const [client, setClient] = useState<Client | null>(null);
  const [configs, setConfigs] = useState<SacmedConfirmationConfig[]>([]);
  const [pending, setPending] = useState<SacmedPendingConfirmation[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [editingConfig, setEditingConfig] = useState<SacmedConfirmationConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [clientData, configsData, pendingData] = await Promise.all([
        ...
        sacmedConfirmationsApi.getConfigs(clientId),
        sacmedConfirmationsApi.getPending(clientId),
      ]);
  ...
  const handleCreateConfig = async () => {
    ...
    try {
      const dto: CreateSacmedConfirmationConfigDto = {
        name: formData.name,
      ...
      await sacmedConfirmationsApi.createConfig(clientId, dto);
  ...
  const handleUpdateConfig = async () => {
    ...
    try {
      await sacmedConfirmationsApi.updateConfig(clientId, editingConfig.id, {
        name: formData.name,
        daysBeforeAppointment: formData.daysBeforeAppointment,
        timeToSend: formData.timeToSend,
        ghlCalendarId: formData.ghlCalendarId,
  ...
  const handleDeleteConfig = async (configId: string) => {
    ...
    try {
      await sacmedConfirmationsApi.deleteConfig(clientId, configId);
  ...
  const startEditing = (config: SacmedConfirmationConfig) => {
  ...
  const handleTrigger = async (configId?: string) => {
    try {
      ...
      const result = await sacmedConfirmationsApi.trigger(clientId, configId ? { configId } : {});
  ...
  const handleProcessSelected = async () => {
    ...
    try {
      ...
      const result = await sacmedConfirmationsApi.processSelected(
        clientId,
  ...
  const handleProcessAll = async () => {
    ...
    try {
      setProcessingAction(true);
      const result = await sacmedConfirmationsApi.processAll(clientId);
      toast.success(
  ...
  const handleValidateGHL = async () => {
    try {
      const result = await sacmedConfirmationsApi.validateGHL(clientId);
      if (result.valid) {
  ...
  const selectAllPending = () => {
    const pendingIds = filteredPending
      .filter((p) => p.status === SacmedConfirmationStatus.PENDING)
      .map((p) => p.id);
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-center h-64">
          <FiLoader className="animate-spin text-3xl text-teal-600" />
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Confirmaciones Sacmed</h1>
              <p className="text-sm text-gray-500 mt-1">
                {client?.name} - Confirmaciones automaticas de citas desde Sacmed via GHL
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="inline-flex items-center px-3 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-teal-600 hover:bg-teal-700"
              >
        ...
        {/* Info banner: Sacmed states */}
        <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
          <div className="flex items-start">
            <FiAlertCircle className="text-teal-600 mt-0.5 mr-2 flex-shrink-0" />
            <div className="text-sm text-teal-700">
              <p className="font-medium">Estados de cita en Sacmed</p>
              <p className="mt-1">
                Sacmed maneja eventos con <span className="font-medium">statusEventId</span> (ej: 2 =
                Confirmado, 7 = Cancelado). El sistema obtiene automaticamente las citas del dia
                objetivo por profesional (fan-out), excluye las canceladas, y sincroniza la
                informacion del contacto en GoHighLevel.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex gap-2">
              ...
              {configs.length < 3 && (
                <button
                  ...
                  className="inline-flex items-center px-3 py-1.5 border border-transparent rounded-md text-sm font-medium text-white bg-teal-600 hover:bg-teal-700"
                >
              ...
              <div key={config.id} className="px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-teal-600 disabled:opacity-50"
          ...
          <div className="bg-white shadow rounded-lg mb-6">
            ...
            <div className="px-6 py-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Orden (1-3)</label>
                  <select
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={
                    processingAction ||
                    filteredPending.filter((p) => p.status === SacmedConfirmationStatus.PENDING)
                      .length === 0
                  }
                  className="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50"
                >
            ...
            <div className="mt-3 flex gap-3">
              ...
              >
                ...
                <option value={SacmedConfirmationStatus.PENDING}>Pendiente</option>
                <option value={SacmedConfirmationStatus.PROCESSING}>Procesando</option>
                <option value={SacmedConfirmationStatus.COMPLETED}>Completada</option>
                <option value={SacmedConfirmationStatus.FAILED}>Fallida</option>
              </select>
              <input
                type="date"
                value={filterDate}
                  ...
                  <Fragment key={confirmation.id}>
                    <tr className="hover:bg-gray-50">
                        ...
                        <button
                          ...
                          title={
                            expandedRows.has(confirmation.id)
                              ? 'Ocultar detalle de ejecución'
                              : 'Ver detalle de ejecución'
                          }
                          aria-label={
                            expandedRows.has(confirmation.id) ? 'Ocultar detalle' : 'Ver detalle'
                          }
                        >
                          {expandedRows.has(confirmation.id) ? (
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">
                        <span className="font-mono text-xs">{confirmation.sacmedEventId}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">
                        {confirmation.appointmentData.modalidad || '-'}
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-500">
