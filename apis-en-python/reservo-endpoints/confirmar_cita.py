...
def lambda_handler(event, context):
  try:
    ...
    if response.status_code != 200:
      return _response(response.status_code, {"status": response.status_code, "message": "Hubo un error al confirmar la cita."})

    return _response(200, {"status": 200, "message": "Cita confirmada con éxito."})
